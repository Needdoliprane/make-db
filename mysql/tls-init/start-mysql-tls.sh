...
# Fichier SQL exécuté à CHAQUE démarrage (idempotent)
pw_sql=$(printf "%s" "$MYSQL_ROOT_PASSWORD" | sed "s/'/''/g")
cat > "$INIT_DIR/root_bootstrap.sql" <<EOF
-- Idempotent: garantit root@% + plugin + password + TLS
...
-- Permettre SSL mais pas l'obliger pour root@% (le transport est déjà forcé globalement)
...
# Première initialisation uniquement (si datadir vierge)
...
# MODIFICATION PRINCIPALE: Retirer --require_secure_transport=ON temporairement
# pour permettre au seeder de se connecter
